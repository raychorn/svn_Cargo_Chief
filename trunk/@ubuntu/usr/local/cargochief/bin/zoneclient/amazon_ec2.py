...
__hops__[__ip__] = traceroute.traceroute(__ip__)

print '%s (%d hops)' % (__ip__,len(__hops__[__ip__]))

#print '%s (%d hops)' % ('maps.google.com',len(traceroute.traceroute('maps.google.com')))
...
for __url__ in __urls__:
    ...
    _ip = _default_ip
    __hops__[_ip] = traceroute.traceroute(_ip)
    print '%s --> %s (%d hops)' % (__url__,_ip,len(__hops__[_ip]))
